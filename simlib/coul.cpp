...
double coul::Penetration(int l, double eta, double x)
{
  init(l,eta,x);
  Shift = (dF*F+dG*G)/(pow(F,2)+pow(G,2));
  return x/(pow(F,2)+pow(G,2));
}
double coul::P_l(double E,double r,int l,double mu,int z1,int z2)
{
  double const kconstant =  .048192;
  double const e2 = 1.44; // Coulomb constant
  double Kwave2 = kconstant*mu*E;
  double Kwave = sqrt(Kwave2);
  double x = r*Kwave;
  double eta = (double)z1*(double)z2*e2*Kwave/E/2.;
  //cout << sqrt(8.*eta*x) << endl;
  return Penetration(l,eta,x);
}
