...
//***************************************
double whit::Shift(double E, double r , int l, double mu, int z1, int z2)
{
  double const kconstant =  .048192;
  double const e2 = 1.44; // Coulomb constant
  double Kwave2 = kconstant*mu*fabs(E);
  double Kwave = sqrt(Kwave2);
  double x = r*Kwave;
  double eta = (double)z1*(double)z2*e2*Kwave/abs(E)/2.;
  //cout << sqrt(8.*eta*x) << endl;
  double W = getWaveFunction(eta, l, x);
  double dW = derivative;
  return x*dW/W;
} 
