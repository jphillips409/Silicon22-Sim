...
pixels::pixels(double dz)
{
  ...
  /*
  ifstream file("/home/Boron8/simlib/pixels.dat");
  ...
  */
  ...
  ifstream file2("/home/Boron8/simlib/hirapixels.dat");
...
void pixels::prepareSim()
{
  ...
  for (int iback =0;iback<32;iback++)
    {
  double distB = ((float)iback +.5)/32.*2. - 1.;
  for (int ifront = 0;ifront<32;ifront++)
    {
     double distF = ((float)ifront +.5)/32.*2. - 1.;

     double x = Tele[itele].r_center[0]  + distF*Tele[itele].r_front[0] + distB*Tele[itele].r_back[0];
     double y = Tele[itele].r_center[1]  + distF*Tele[itele].r_front[1] + distB*Tele[itele].r_back[1];
     double z = Tele[itele].r_center[2]  + distF*Tele[itele].r_front[2] + distB*Tele[itele].r_back[2];

     if (fabs(z-TeleP[itele].Location[ifront][iback].z) > .001 || (ifront == 3 && iback == 17))
       {
	 cout << ifront << " " << iback << " " << TeleP[itele].Location[ifront][iback].x << " " << x << " " << TeleP[itele].Location[ifront][iback].y << " " << y << " " << 
       TeleP[itele].Location[ifront][iback].z << " " << z << endl; 
       }
    }
    }
  */
...
//************************************************
bool pixels::sim(float theta0, float phi0, float xtarget, float ytarget, float rand1, float rand2, float dz)
{
  //cout << theta0 << endl;
  bool hitit = false;
  sle Sle(3);
  for (int itele=0;itele<14;itele++)
    {
     ...
     float dist_back = Sle.Y[2];
      ...
      r_insect1[0] = xtarget + Sle.Y[0]*sin(theta0)*cos(phi0);
      r_insect1[1] = ytarget + Sle.Y[0]*sin(theta0)*sin(phi0);
      r_insect1[2] = dz + Sle.Y[0]*cos(theta0);



      r_insect2[0] = Tele[itele].r_center[0] + Sle.Y[1]*Tele[itele].r_front[0] + Sle.Y[2]*Tele[itele].r_back[0];


      r_insect2[1] = Tele[itele].r_center[1] + Sle.Y[1]*Tele[itele].r_front[1] + Sle.Y[2]*Tele[itele].r_back[1];


      r_insect2[2] = Tele[itele].r_center[2] + Sle.Y[1]*Tele[itele].r_front[2] + Sle.Y[2]*Tele[itele].r_back[2];


      double rr_insect = sqrt(pow(r_insect1[0],2) + pow(r_insect1[1],2) + pow(r_insect1[2],2));

      double theta_c = acos(r_insect1[2]/rr_insect);
      double phi_c = atan2(r_insect1[1],r_insect1[0]);
      ...
      */
       ...
       */
       thetaRecon = getAngle(itele,ixStrip,iyStrip);
       phiRecon = phi;
