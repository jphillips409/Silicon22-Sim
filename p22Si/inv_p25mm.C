{
 gROOT->Reset();
  ...
  tt.DrawLatex(.1,420/2,"0.25mm target");
  tt.DrawLatex(.1,360/2,"10% branch");

  tt.DrawLatex(1.,260/2,"all");
  tt.SetTextColor(2);
  tt.DrawLatex(.8,150/2,"|cos#theta_{H}|<0.5");
  tt.SetTextColor(4);
  tt.DrawLatex(.8,70/2,"|cos#theta_{H}|<0.2");
 
  TArrow arrow;
  arrow.SetAngle(30);
  arrow.SetLineColor(6);
  arrow.SetFillColor(6);
  arrow.DrawArrow(.475,300/2,.475,250/2,.03);
  arrow.DrawArrow(.475+4.3-3.045,550/2,.475+4.3-3.045,500/2,.03);
  
  tt.SetTextColor(6);
  tt.DrawLatex(.54,261/2,"2^{+}");
  tt.DrawLatex(1.85,497/2,"1^{+}");
}
