...
gROOT->ForceStyle();
 ...
 TCanvas canvas("level_22Si_Michel");
    double delta = .06;
  ...
  //HO assumption for first 4+ state
  ll.DrawLine(xmin22Si,3.2+0.166,xmax22Si,3.2+0.166);
  //box23.Draw("same");
  ...
  tt.DrawLatex(.079,3.52,"4^{+}");

  tt.DrawLatex(.3,0.8,"5/2^{+}");
  ...
  tt.SetTextSize(.08);
  tt.DrawLatex(.5,3.6,"GSM");
  TArrow arrow;
  arrow.SetFillColor(2);
  arrow.SetLineColor(2);
  arrow.SetAngle(30);
  arrow.SetArrowSize(0.03);
  arrow.DrawArrow(xmax22Si-.05,1.766,xmin21Al,1.088);
  arrow.DrawArrow(xmax22Si-.05,3.2+0.166,xmin21Al,1.088);
