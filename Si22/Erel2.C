{
  ...
  gROOT->ForceStyle();
  TCanvas canvas("Erel_22Si","");

  TFile file("sim.root");
  TH1I* hist = (TH1I*) file.Get("Erel_trans");
  TH2S frame("frame","",10,0,2.8,10,0,500);
  ...
  TArrow arrow;
  arrow.SetAngle(30);
  arrow.DrawArrow(1.761,240*2,1.7610,200*2);
  arrow.DrawArrow(1.761+.69,50*2,1.761+.69,10*2);
  //arrow.DrawArrow(2.327,80,2.327,30);
  //arrow.DrawArrow(.694,150,.694,60);

  TLatex tt;
  tt.DrawLatex(1.81,213*2,"2^{+}");
  tt.DrawLatex(2.40,58*2,"0^{+}_{2}");
  // tt.DrawLatex(2.258,91,"3/2^{+}");
  //tt.DrawLatex(.629,177,"3/2^{+}");
  tt.DrawLatex(.39,200*2,"^{22}Si#rightarrow2#it{p}+^{20}Mg");
}
