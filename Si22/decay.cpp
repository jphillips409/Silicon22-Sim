...
double CDecay::getErelRel(CFrame **part)
{
  ...
  ErelRecon = 0.;
   ...
   for(int i =0;i<3;i++)
     {
       vv += pow(partCM[2]->v[i],2);
     }
   vv = sqrt(vv);
   cos_thetaC = partCM[2]->v[2]/vv;
...
void CDecay::ModeSequential_4plus_GSM_HO()

{

  double Ek1 = 2.278; //Harmonic Oscillator state, twice the E* of the 2+ state. Assume d-wave decay
  double Ek2 = 1.074;
...
void CDecay::ModeSequential_0plus()

{
  double Ek2 = 0.;
  double Ek1 = 0.;
  gamma_emitted = false;
  double E_parent0 = 2.146;
  double gamma_parent = .008;
  double E_inter0 = 1.5;
  double gamma_inter = .081;

  for(;;)
    {
      double E_parent = ran.BreitWigner(E_parent0,gamma_parent);
      double E_inter = ran.BreitWigner(E_inter0,gamma_inter);

      Ek1 = E_parent - E_inter;
      Ek2 = E_inter;
      //cout << Ek1 + Ek2 << endl;      
      if (Ek1 > 0. && Ek2 > 0. && Ek1 < 7. && Ek2 < 7.)break;
    }
