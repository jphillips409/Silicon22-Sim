{
  ...
  TH2S frame("frame","",10,0,6.5,10,0,60);
  ...
  tt.DrawLatex(0.5,40,"#splitline{(a) ^{23}P#rightarrow3#it{p}+^{20}Mg}{     85 MeV/A}");
  tt.DrawLatex(0.5,28,"     87 mm");

  canvas.cd(2);
  TH2S frame2("frame2","",10,0,6.5,10,0,60);
  ...
  hist3->Rebin(25);
  hist3->Draw("same"); 
  tt.DrawLatex(0.5,40,"(b) 100 mm");

  canvas.cd(3);
  TH2S frame3("frame3","",10,0,6.5,10,0,60);
  frame3.GetXaxis()->SetTitle("Q_{p} (MeV)");

  frame3.GetYaxis()->SetTitle("Counts/250 keV");
  frame3.GetYaxis()->CenterTitle();
  frame3.GetXaxis()->CenterTitle();
  frame3.Draw();

  TFile file4("sim_85MeVA_fibdist110.root");
  TH1I* hist4 = (TH1I*) file4.Get("Erel");

  hist4->Rebin(25);
  hist4->Draw("same"); 
  tt.DrawLatex(0.5,40,"(c) 110 mm");

  canvas.cd(4);
  TH2S frame4("frame4","",10,0,6.5,10,0,60);
  frame4.GetXaxis()->SetTitle("Q_{p} (MeV)");

  frame4.GetYaxis()->SetTitle("Counts/250 keV");
  frame4.GetYaxis()->CenterTitle();
  frame4.GetXaxis()->CenterTitle();
  frame4.Draw();

  TFile file5("sim_85MeVA_fibdist120.root");
  TH1I* hist5 = (TH1I*) file5.Get("Erel");

  hist5->Rebin(25);
  hist5->Draw("same"); 
  tt.DrawLatex(0.5,40,"(d) 120 mm");

  canvas.cd(5);
  TH2S frame5("frame5","",10,0,6.5,10,0,60);
  frame5.GetXaxis()->SetTitle("Q_{p} (MeV)");

  frame5.GetYaxis()->SetTitle("Counts/250 keV");
  frame5.GetYaxis()->CenterTitle();
  frame5.GetXaxis()->CenterTitle();
  frame5.Draw();

  TFile file6("sim_85MeVA_fibdist160.root");
  TH1I* hist6 = (TH1I*) file6.Get("Erel");

  hist6->Rebin(25);
  hist6->Draw("same"); 
  tt.DrawLatex(0.5,40,"(e) 160 mm");
